...
type Account struct {
	...
	Role                  *int32   `json:"role,omitempty"`       // User.id
	Instrument            *int32   `json:"instrument,omitempty"` // Instrument.id
	Company               *int32   `json:"company,omitempty"`    // Company.id
	...
	SyncID                []string `json:"syncID,omitempty"`                // Массив банковских номеров счета. Обычно берутся последние 4 цифры номера счета и последние 4 цифры номеров банковских карт, привязанных к счету.
	Balance               *float64 `json:"balance,omitempty"`               // Текущий баланс счета
	StartBalance          *float64 `json:"startBalance,omitempty"`          // Баланс счета в момент открытия. Если тип счета - кредит, то указывается тело кредита.
	CreditLimit           *float64 `json:"creditLimit,omitempty"`           // >= 0. Кредитный лимит в случае, если тип счета - банковская карта или банковский счет.
	...
	Savings               *bool    `json:"savings,omitempty"`               // Является ли счёт накопительным.
	...
	Capitalization        *bool    `json:"capitalization,omitempty"`        // Для депозита - есть ли капитализация процентов. Для кредита - является ли кредит аннуитетным.
	Percent               *float64 `json:"percent,omitempty"`               // >= 0 && < 100. Процентная ставка по счету (в процентах).
	StartDate             string   `json:"startDate,omitempty"`             // 'yyyy-MM-dd'. Дата открытия депозита / кредита.
	EndDateOffset         *int32   `json:"endDateOffset,omitempty"`         // Срок действия кредита / депозита в промежутках endDateOffsetInterval начиная с даты открытия.
	EndDateOffsetInterval string   `json:"endDateOffsetInterval,omitempty"` // ('day' | 'week' | 'month' | 'year'). Промежуток между выплатами. Может быть null, тогда считается, что выплата процентов или погашение кредита происходит в конце срока.
	PayoffStep            *int32   `json:"payoffStep,omitempty"`            // Hаз в сколько payoffInterval происходят выплаты, начиная со следующей даты startDate + payoffInterval. Если payoffInterval == null, то значение должно быть равно 0.
	PayoffInterval        *string  `json:"payoffInterval,omitempty"`        // ('month' | 'year')?
}
...
type Tag struct {
	...
	User          int     `json:"user"`    // User.id
	Title         string  `json:"title"`
	Parent        *string `json:"parent,omitempty"`   // Tag.id. Родительская категория. Допускается степень вложенности не больше 1, т.е. у категории может быть родительская категория, а у родительской категории уже не может быть своего родителя.
	Icon          *string `json:"icon,omitempty"`     // Id иконки категории
	Picture       *string `json:"picture,omitempty"`  // Ссылка на картинку для данной категории
	Color         *int32  `json:"color,omitempty"`    // цвет иконки категории в в виде числа. Рассчитывается по alpha, red, green, blue 0 <= 255. unsigned long color = (a << 24) + (r << 16) + (g << 8) + (b << 0).
	...
	Required      *bool   `json:"required,omitempty"` // Являются ли расходы по данной категории обязательными. Если null, то тоже считаются обязательными
}
...
type Reminder struct {
	...
	Merchant          *string  `json:"merchant,omitempty"` // Merchant.id
	Payee             string   `json:"payee"`
	Comment           string   `json:"comment"`
	Interval          *string  `json:"interval,omitempty"` // ('day' | 'week' | 'month' | 'year'). Интервал шага. Если null, значит, планируемая без повторения.
	Step              *int32   `json:"step,omitempty"`     // >= 0. Шаг с которым создаются планируемые операции
	Points            []int    `json:"points,omitempty"`   // >= 0 && < step. Точки внутри шага, в которых создаются планируемые.
	StartDate         string   `json:"startDate"`          // 'yyyy-MM-dd'. с какой даты создавать планируемые.
	EndDate           *string  `json:"endDate,omitempty"`  // 'yyyy-MM-dd'. До какой даты включительно создавать планируемые. Если null, то бессрочно.
...
type Budget struct {
	Changed     int     `json:"changed"`       // Unix timestamp
	User        int     `json:"user"`          // User.id
	Tag         *string `json:"tag,omitempty"` // Tag.id | '00000000-0000-0000-0000-000000000000'. Категория бюджета. Если null, то это бюджет по операциям без категории. Если '00000000-0000-0000-0000-000000000000', то это бюджет совокупный за месяц.
